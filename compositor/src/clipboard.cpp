...
void source_send(wlr_data_source *source, const char *mime_type, int32_t fd) {
  auto *self = reinterpret_cast<TextSource *>(source);
  (void)mime_type; // Every type this offers is the same bytes.

  // Non-blocking, because the writer below is allowed to find the pipe full
  // and come back to it.
  const int flags = fcntl(fd, F_GETFL, 0);
  if (flags != -1) fcntl(fd, F_SETFL, flags | O_NONBLOCK);

  auto *pending = new PendingWrite{self->text, 0, nullptr};
  if (g_loop != nullptr) {
    pending->source =
...
/// Whether a source is one of ours — which is what lets `get` answer from
...
/// The first MIME type `source` offers that this understands, or nullptr.
///
...
const char *preferredMime(const wlr_data_source *source) {
  const auto *offered = static_cast<const char *const *>(source->mime_types.data);
...
std::string Clipboard::get() {
  ...
  const char *mime = preferredMime(source);
  if (mime == nullptr) return {}; // An image, a file list: nothing to paste.

  int fds[2];
  if (pipe(fds) != 0) return {};

  wlr_data_source_send(source, mime, fds[1]); // takes fds[1] and closes it
