...
struct SymbolPageView {
    ...
    init(page: Page, index: DocsIndex) {
        ...
        lang_title = language.map(Site.title(of:)) ?? page.lang
        title = page.title
        ...
        for n in 1..<max(page.components.count, 1) {
            if let p = index.page(lang: page.lang, components: Array(page.components.prefix(n))) {
                ...
                // A module or namespace: its listing, which the API route
                // serves for any namespace without a page of its own.
                crumbs.append(Crumb(title: page.components[n - 1],
                                    url: "/api/\(page.lang)/" + page.components.prefix(n).joined(separator: "/")))
            }
        }
        self.crumbs = crumbs
...
struct LangView {
    ...
    /// Every namespace of `language`, or the one named `namespace` (a Swift
    /// module).
    init(language: Language, index: DocsIndex, namespace: String? = nil) {
        if let namespace {
            title = namespace
            (symbols, documented) = Site.stats(index, lang: language, namespace: namespace)
        } else {
            title = Site.title(of: language) + " API"
            (symbols, documented) = index.stats(lang: language.id)
        }
        namespaces = index.topLevelPages(lang: language.id)
            .filter { group in
                namespace.map { group.namespace == $0 || group.namespace.hasPrefix($0 + language.separator) } ?? true
            }
            .map { group in
            NamespaceView(
    ...
    init(query: String, index: DocsIndex, limit: Int) {
        ...
        hits = index.search(query, limit: limit).map { hit in
            HitView(name: hit.symbol.name, qualified: hit.symbol.qualified,
                    ...
                    lang_title: Language.named(hit.symbol.lang).map(Site.title(of:)) ?? hit.symbol.lang,
