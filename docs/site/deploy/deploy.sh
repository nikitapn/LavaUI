#!/bin/bash
# Deploy the LavaUI documentation site to a Docker host behind npquicrouter.
# Run from anywhere in the LavaUI checkout; see usage below. The same script
# as nprpc/docs/site/deploy, with LavaUI's names and its own api.json build.
...
SSH_TARGET=""
HOSTNAME="lavaui.nikitapn.com"
# nprpc.nikitapn.com is on 9443.
PORT="9444"
CERT_DIR="/etc/letsencrypt"
CERT_NAME=""
CONTAINER_NAME="lavaui-docs"
IMAGE_NAME="lavaui-docs:latest"
...
routes the hostname to --port on the server's loopback.
  ...
  --hostname <name>         Public hostname (default: lavaui.nikitapn.com)
  ...
  --container <name>        Container name (default: lavaui-docs)
  --image <name>            Image tag built on the server (default: lavaui-docs:latest)
...
cd "$REPO_DIR"
API_JSON="$REPO_DIR/.build/docs/api.json"
...
$RUNTIME_TAG was not built from $DEV_IMAGE (their libnprpc differs):
  ...
  dev:     $DEV_LIB_SHA
Rebuild it (in nprpc): just build-runtime-image $DEV_IMAGE
MSG
  exit 1
fi
...
# On the host, not in the image: the symbol graphs come from compiling LavaUI,
# which needs Vulkan, GLFW and the rest of its packages.
if [ "$BUILD_API" = 1 ]; then
  "$REPO_DIR/scripts/docs-api.sh"
fi
[ -f "$API_JSON" ] || { echo "Missing $API_JSON; run scripts/docs-api.sh" >&2; exit 1; }
...
scp "$STAGE/lavaui-docs.tar.gz" "$SSH_TARGET:$REMOTE_TMP_DIR.tar.gz"
